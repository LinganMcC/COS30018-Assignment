...
Every training run appends one row to experiments/experiment_log.csv so
the final report's comparison table can be built directly from real data.
Row schema matches what was agreed with Liam so his Task 4 evaluation
harness can consume this file without reformatting.
...
-----
    ...
    log_run(
        ...
        training_time_s=52.4,
        notes="baseline; EarlyStopping stopped at epoch 12",
    )
"""
...
# Column order - do not change without telling Liam.
FIELDS = [
    "timestamp",
    "run_id",
    "architecture",
